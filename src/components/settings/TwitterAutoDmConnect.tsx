...
export function TwitterAutoDmConnect({
  connections,
  ready,
  onChanged,
}: {
  connections: TwitterAutoDmAccount[];
  ready: boolean;
  onChanged: () => void;
}) {
  ...
  const connect = useMutation({
    ...
    onSuccess: ({ url }) => {
      window.sessionStorage.setItem(
        ...
        TWITTER_CONNECTION_RETURN_TO.autoDm,
      );
      window.location.assign(url);
    },
    onError: (error) =>
      toast.error(error instanceof Error ? error.message : "Could not start X connection"),
  });
  const disconnect = useMutation({
    ...
    onError: (error) =>
      toast.error(error instanceof Error ? error.message : "Could not disconnect X"),
  });

  return (
    <div className="space-y-3 sm:max-w-md">
      <p className={micro.mutedXs}>
        One X login powers Auto DMs and Social scheduling. Approve Direct Message access when X
        asks.
      </p>
      <div className="rounded-[22px] border border-black/[0.06] bg-white p-4 shadow-sm">
        <div className="flex items-center gap-3">
          <span className="flex size-11 shrink-0 items-center justify-center rounded-2xl bg-white shadow-sm ring-1 ring-black/[0.06]">
        ...
        {connections.map((connection) => (
          ...
          >
            ...
            {!connection.canAutomate && (
              <span className="hidden text-[#b7790b] sm:inline">Reconnect for DM access</span>
            )}
            <button
              type="button"
              ...
              onClick={() => {
                if (
                  window.confirm(
                    `Disconnect ${connection.displayName}? Its automations and scheduled X posts will stop using this account.`,
                  )
                ) {
