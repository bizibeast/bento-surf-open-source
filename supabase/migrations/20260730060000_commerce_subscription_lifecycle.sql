-- Normalized recurring-commerce lifecycle.
--
-- Paid orders are immutable financial history. Subscription state lives here
-- and controls the single durable access grant created by the first payment.
create table if not exists public.commerce_subscription_access (
  ...
  status text not null default 'active'
    check (status in ('active', 'cancel_at_period_end', 'past_due', 'expired', 'revoked')),
  ...
  unique (provider, provider_account_id, provider_subscription_id)
);

create index if not exists commerce_subscription_access_creator_idx
  on public.commerce_subscription_access(creator_id, updated_at desc);
create index if not exists commerce_subscription_access_expiration_idx
  on public.commerce_subscription_access(
    coalesce(grace_expires_at, current_period_end)
  )
  where status in ('active', 'cancel_at_period_end', 'past_due');

alter table public.commerce_subscription_access enable row level security;
revoke all on table public.commerce_subscription_access from public, anon, authenticated;
grant all on table public.commerce_subscription_access to service_role;

drop trigger if exists commerce_subscription_access_updated_at
  on public.commerce_subscription_access;
create trigger commerce_subscription_access_updated_at
  before update on public.commerce_subscription_access
...
begin
  ...
  -- Never let a delayed webhook shorten a newer paid billing period.
...
create or replace function public.expire_commerce_subscription_access(
...
begin
  with expired_rows as (
    update public.commerce_subscription_access
       set status = 'expired'
     where status in ('active', 'cancel_at_period_end', 'past_due')
       and coalesce(grace_expires_at, current_period_end) is not null
       and coalesce(grace_expires_at, current_period_end) <= p_now
    returning access_grant_id
  ),
  expired_grants as (
    update public.commerce_access_grants grant_row
       set status = 'expired',
           expires_at = coalesce(grant_row.expires_at, p_now)
     where grant_row.id in (
       select access_grant_id from expired_rows where access_grant_id is not null
     )
...
revoke all on function public.expire_commerce_subscription_access(timestamptz)
  from public, anon, authenticated;
grant execute on function public.expire_commerce_subscription_access(timestamptz)
  to service_role;

-- Existing paid subscriptions keep their current access grant. Their exact
-- billing period is populated by the next provider lifecycle webhook.
insert into public.commerce_subscription_access(
  provider,
  provider_account_id,
  provider_subscription_id,
...
select distinct on (
    order_row.provider,
    coalesce(order_row.provider_account_id, ''),
    order_row.provider_subscription_id
  )
  order_row.provider,
  coalesce(order_row.provider_account_id, ''),
  order_row.provider_subscription_id,
...
left join public.commerce_access_grants grant_row
  on grant_row.order_id = order_row.id
where order_row.status = 'paid'
  and order_row.provider_subscription_id is not null
order by
  order_row.provider,
  coalesce(order_row.provider_account_id, ''),
  order_row.provider_subscription_id,
  (grant_row.id is not null) desc,
  order_row.paid_at asc nulls last
on conflict (provider, provider_account_id, provider_subscription_id) do nothing;

comment on table public.commerce_subscription_access is
  'Normalized provider subscription lifecycle controlling a durable commerce access grant.';
